...
from ...core import graph, filters
...
class Context:
    ...
    @staticmethod
    def from_tree(tree, *, map=None, is_leaf=None):
        import jax.tree
        from ...core import graph_util

        map = map if map is not None else (lambda x: x)

        filter = filters.IsLeaf()
        filter = (
            filters.Any(lambda p, v: is_leaf(v), filter)
            if is_leaf is not None
            else filter
        )
        _, leaves = graph.split(tree, filter)
        items = {
            ".".join(str(k) for k in path): jax.tree.map(map, v)
